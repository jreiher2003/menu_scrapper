...
def index():
    ...
    # flo = County.query.filter_by(state_id=10).all()
    # flo_citys = City.query.filter_by(state_id=10).all()
    return render_template(
        "index.html",
         state=state)
         # flo=flo,
         # flo_citys=flo_citys,
         # fc = zip(flo,flo_citys),
         # state_county=zip(state,flo))
...
def state_page(url_slug_state,state_id):
    ...
    city = City.query.filter_by(state_id=state_id, metro_area=False).all()
    metro = MetroArea.query.filter_by(state_id=state_id).all()
    state_name = State.query.filter_by(id=state_id).one()
    return render_template("state_page.html", url_slug_state=url_slug_state,state_name=state_name.name, state_id=state_id, county=county, city=city, metro=metro)
